...
param(
    ...
    [string]$Repo = "https://github.com/marshaljlee/jxproxy.git",
    [switch]$MinClone,
    [switch]$NoBuild,
    [switch]$Help
)
...
switch ($arch) {
    0 { $cpuArch = "x86"; $target = "bun-windows-x64" }      # x86
    9 { $cpuArch = "x64"; $target = "bun-windows-x64" }      # x64 (AMD64/Intel 64)
    12 { $cpuArch = "ARM64"; $target = "bun-windows-arm64" } # ARM64 (Snapdragon X, Surface, etc.)
    5 { $cpuArch = "ARM"; $target = "bun-windows-arm64" }    # ARM32
    default { $cpuArch = "x64"; $target = "bun-windows-x64" }
}
Write-Host "  Architecture: $cpuArch → build target: $target" -ForegroundColor Cyan
Write-Host ""
...
set "JXPROXY_PORT=5529"
set "JXPROXY_AUTH_TOKEN=jxproxy"
